...
@Component({
    ...
    template: ` <div class="d-inline-flex flex-wrap">
        <ng-container *ngIf="goTitle">
            <div
                (click)="page.invalid || sendChange(page.value)"
        ...
        <div tsToggle [target]="menuView" [bind]="menuView" class="pagination-button pagination-button-{{ color }}">
            {{ getLimitText(pagination.limit) }}
        </div>
        <div
            #menuView="tsView"
            style="z-index:9999"
        ...
        >
            <button *ngFor="let item of items" (click)="changeLimit(item.value)" class="dropdown-item" close>
                {{ item.text }}
            </button>
        </div>
        <div
        ...
        <ng-template ngFor let-item [ngForOf]="pages" let-i="index">
            <div
                (click)="pagination.page = item; sendChange()"
                [ngClass]="{ active: item == pagination.page }"
                class="pagination-item pagination-item-{{ color }}"
            >
                {{ item }}
            </div>
        </ng-template>
