...
export class ToastService {
    ...
    constructor(@Inject('DEFAULT_CONFIG') private config: ToastConfig, private cmpService: ComponentService) {}

    init(): void {
        this.handler || (this.handler = this.cmpService.create(ToastComponent));
    }

    create(title: string, message: string, options: { color: string; icon?: string; timeout?: number }): void {
        this.init();
        this.handler.instance.addToast(
            new Toast(title, message, options.color, options.icon || '', options.timeout || this.config.timeout)
        );
    }
    ...
    notify(options: NotifyConfig): void {
