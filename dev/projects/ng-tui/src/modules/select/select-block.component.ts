  ...
  writeValue(values: Array<any>): void {
    if (Array.isArray(values)) {
      ...
      this.values.forEach((value, i) => {
        ...
        this.activeOptions[i] = this.optionsQuery[i].find(
          (option) => option.value === value
        ) as any;
        // tslint:disable-next-line:no-unused-expression
        this.activeOptions[i] &&
          (this.optionsQuery[i + 1] = this.activeOptions[i].children as any);
      });
      this.title = this.activeOptions
        .map((option) => (option ? option.text : ''))
        .join(' / ');
    }
  }

  showChildren(i: number, node: ItemTree): void {
